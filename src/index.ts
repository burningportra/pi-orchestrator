...
export default function (pi: ExtensionAPI) {
  ...
  // runGuidedGates is now in src/gates.ts — imported at the top.
  // Shim: create an OrchestratorContext for the extracted function.
  function makeOC(): import("./types.js").OrchestratorContext {
    return {
      pi,
      state,
      get orchestratorActive() { return orchestratorActive; },
      set orchestratorActive(v: boolean) { orchestratorActive = v; },
      version: ORCHESTRATOR_VERSION,
      sophiaCRResult,
      worktreePool,
      swarmTender,
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (state.phase === "iterating" && params.beadId === "__gates__") {
        return await runGuidedGates(makeOC(), state, ctx, "");
      }
      ...
      if (params.verdict === "pass") {
        ...
        if (ready.length === 0) {
          ...
          return await runGuidedGates(makeOC(), state, ctx, beadsReviewInfo);
